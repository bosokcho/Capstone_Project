...
hurricanes_per_year2 <- hurricane_data %>%
  group_by(YEAR) %>%
  summarise(TOTAL_H = n_distinct(NAME))

names(hurricanes_per_year2)
plot(TOTAL_H ~ YEAR, data = hurricanes_per_year)
...
var(hurricane_freq_sample1$beta0) 
var(hurricane_freq_sample1$beta1)
var(hurricane_freq_sample1$beta2)
var(hurricane_freq_sample1$beta3)
...
```{r}
library(ggplot2)
ggplot(elect_samples_2, aes(x=theta.1.)) + 
  geom_histogram(color="white", boundary=0, aes(y=..density..)) +
  lims(x=c(-0.5,0.5))
ggplot(elect_samples_2, aes(x=theta.5.)) + 
  geom_histogram(color="white", boundary=0, aes(y=..density..)) +
  lims(x=c(-0.5,0.5))
```
