...
windSpeed_model <- "model{
    #Data
    for(i in 1:length(y)) {
        y[i] ~ dnorm(beta0 + beta1*c[i] + beta2*t[i] + beta3*n[i], tau)
...
#*combining the model (games_model) and data (x)
#*set the random number seed
windSpeed_jags <- jags.model(textConnection(windSpeed_model),data=list(y=HURRICANES$avg_windspeed,c=HURRICANES$avg_CO2, t=HURRICANES$temp,n=HURRICANES$avg_NAO),
                    inits=list(.RNG.name="base::Wichmann-Hill", .RNG.seed=1989))
