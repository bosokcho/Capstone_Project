...
<<<<<<< HEAD
ggplot(hurricanes_per_year, aes(x = YEAR, y = TOTAL_HURRICANES)) + geom_bar(stat = 'identity', fill = 'skyblue3', col = "white") + xlab("Year") + ylab("Total Hurricanes")
=======
...
ggplot(hurricanes_categories, aes(x = YEAR, y = TOTAL_HURRICANES)) + geom_bar(stat = "identity")+ ylab("Total Hurricanes")
```
...
ggplot(CO2, aes(Date, Average)) + geom_line()+xlab("Year")+ylab("C02 concentration")+theme(axis.title = element_text(size=20))

# Add trend line
ggplot(CO2, aes(Date, Average)) + geom_line() + geom_smooth()+xlab("CO2 Emissions")+ylab("Total Number of Hurricanes")+theme(axis.title = element_text(size=20))
...
# plot
ggplot(data = NAO_avgMonth, aes(x = Group.1, y = x)) + geom_point(color = "blue") + xlab("Months") + ylab("NAO index")+theme(axis.title = element_text(size=20))
```
