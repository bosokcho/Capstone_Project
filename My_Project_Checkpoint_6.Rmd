...
data_jags = as.list(foo[,2])
str(data_jags)

duration_jags <- jags.model(textConnection(hur_mod),data=list(y = foo$Average_Duration, X1 = hurricanes_per_year_clean$temp, X2 = hurricanes_per_year_clean$avg_CO2,X3 = hurricanes_per_year_clean$avg_NAO), inits=list(.RNG.name="base::Wichmann-Hill", .RNG.seed=2000))
